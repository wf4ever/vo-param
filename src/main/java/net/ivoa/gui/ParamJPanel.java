...
public class ParamJPanel extends JPanel {
	...
	public ParamJPanel(SingleParameter param) {
		...
		this.paramName = new JLabel(this.param.getName() + " ( "
				+ this.param.getUnit() + "; " + this.param.getParameterType()
				+ ")");
	...
	public GeneralParameter verify() {
		GeneralParameter gp = new GeneralParameter(this.paramValue.getText(),
				this.param.getParameterType().name(),
				this.param.getSkossConcept(), new GeneralParameterVisitor());
		return gp;
	}
