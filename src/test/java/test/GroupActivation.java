...
public class GroupActivation extends BaseExample {
	...
	protected Service buildService() {
		...
		Service service = factory.createService().withServiceId("CMZ_PROTO_01")
...
*/
		...
		SingleParameter coordtyp = new SingleParameter()
				.withName("coordsystem")
				.withParameterType(ParameterType.STRING);
...
*/
		...
		AbstractCriterion crit = new Criterion().withConditionType(abcon)
				.withExpression(ctref);
		WhenConditionalStatement whenstmt = new WhenConditionalStatement()
				.withWhen(new When(crit));
		speed.setActive(whenstmt);

		speedsp.setActive(new WhenConditionalStatement().withWhen(new When()
				.withCriterion(new Criterion().withConditionType(
						new BelongToSet().withValue(mkconst("spherical")))
						.withExpression(ctref))));

		SingleParameter modeltype = new SingleParameter().withName("model")
				.withParameterType(ParameterType.STRING);
		AbstractCriterion crit2 = new Criterion()
				.withConditionType(new BelongToSet().withValue(
						mkconst("relativistic"), mkconst("non-relativistic")));
		// AlwaysConditionalStatement alwy = new
		// AlwaysConditionalStatement().withAlways(new Always(crit));
		...
		Operation plusVZ2 = factory.createOperation()
				.withOperationType(OperationType.PLUS).withExpression(vz2);

		vy2.setOperation(plusVZ2);

		Operation plusVY2 = factory.createOperation()
				.withOperationType(OperationType.PLUS).withExpression(vy2);
		...
		ValueLargerThan vlt2 = new ValueLargerThan(mktconst("0",
				ParameterType.REAL), true);
		ValueSmallerThan vst2 = new ValueSmallerThan(mktconst("299792458",
				ParameterType.REAL), false);
		...
		alwaysStatement
				.setComment("The norm of speed must be smaller than c and greater than 0");
		...
		// ValueLargerThan vltime = new
		// ValueLargerThan(mktconst("0",ParameterType.REAL), true);
		...
		// DefaultValue vltime = new
		// DefaultValue().withValue(mktconst("10",ParameterType.REAL));

		List<String> values = new ArrayList<String>();
		values.add("true");
		values.add("true");

		DefaultValue vltime = new DefaultValue().withValue(mkVectorConstant(
				values, ParameterType.BOOLEAN));
		...
		ValueLargerThan vlmass = factory.createValueLargerThan()
				.withValue(mktconst("0", ParameterType.REAL))
				.withReached(false);
		...
		massAndTimeAlwaysStatement
				.setComment("Mass and time must always be positive (not strictly)");
		...
		ValueLargerThan vlkinetic = new ValueLargerThan(mktconst("0",
				ParameterType.REAL), true);
