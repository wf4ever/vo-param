...
straightforward :
the result is equal to the boolean-evaluation of the couple $(\mathcal E, \mathcal C)$. This couple
is evaluated according to the concrete class involved, as explained in paragraphs \ref{par-IsNull}
to \ref{par-DefaultValue} \\
It is a bit more complex when criterions contains {\it LogicalConnectors}. Let us see how to
proceed.\\
To begin with, let us consider only {\it Criterion} concrete objects:\\
As we saw in the previous paragraphs, criterions object are (with the help of {\it
...
composing the structure. The term $LC_i$ denotes the {\it LogicalConnector} contained into the
criterion $\mathcal{CR}_i$.\\
As we saw in paragraphs \ref{par-IsNull} to \ref{par-DefaultValue} every couple $(\mathcal E_i,
...
Let us now consider {\it ParenthesisCriterion}  criterions. A representation of such a criterion
...
The criterion structure contained into $\langle \cdot \rangle_{\mathcal{CR}}$ has the highest
...
\cdot \rangle_{\mathcal{CR}_d}$ contains only criterions of type {\it Criterion}.Thus one can simply
